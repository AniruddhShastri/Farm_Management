...
/**
 * Hybrid RES Calculator logic extracted from Bio gas kartas.rtf
 */
export function getRTFMethane(cattleType = 'Dairy', numAnimals = 0, amountBiomassTonsDay = 0) {
  let B = 0;
  if (cattleType === 'Dairy') {
    if (numAnimals !== 0 && amountBiomassTonsDay === 0) {
      B = numAnimals * 0.03 * 0.18 * ((0.396 * 400) + (0.16 * 0.181 * 497));
    } else if (numAnimals === 0 && amountBiomassTonsDay !== 0) {
      B = amountBiomassTonsDay * 0.18 * ((0.396 * 400) + (0.16 * 0.181 * 497));
    }
  } else if (cattleType === 'Beef') {
    if (numAnimals !== 0 && amountBiomassTonsDay === 0) {
      B = numAnimals * 0.029 * 0.18 * ((0.393 * 400) + (0.16 * 0.121 * 497));
    } else if (numAnimals === 0 && amountBiomassTonsDay !== 0) {
      B = amountBiomassTonsDay * 0.18 * ((0.393 * 400) + (0.16 * 0.121 * 497));
    }
  } else if (cattleType === 'Feedlot') {
    if (numAnimals !== 0 && amountBiomassTonsDay === 0) {
      B = numAnimals * 0.03 * 0.18 * ((0.356 * 400) + (0.16 * 0.17 * 497));
    } else if (numAnimals === 0 && amountBiomassTonsDay !== 0) {
      B = amountBiomassTonsDay * 0.18 * ((0.356 * 400) + (0.16 * 0.17 * 497));
    }
  }
  return B; // m^3
}

export function getRTFPower(methaneM3) {
  // Constant from MATLAB formula E = B * 22626 * 0.0002973071 * 0.35
  const E = methaneM3 * 22626 * 0.0002973071 * 0.35;
  return E; // kWh
}

export function getHybridRESSuggestion(dailyPvKwh, dailyLoadKwh, biogasGeneratedKwh, isAddingBiomass = true, cattleType = 'Dairy') {
  if (dailyPvKwh >= dailyLoadKwh) {
    return {
      status: 'success',
      title: 'Solar Sufficient',
      message: 'Solar power can satisfy the load, no need for biogas generator.',
      surplus_solar: dailyPvKwh - dailyLoadKwh,
      biogasUsed: 0
    };
  }

  const biogasPowerNeeded = dailyLoadKwh - dailyPvKwh;
  const remainingBiogasPower = biogasGeneratedKwh - biogasPowerNeeded;

  if (remainingBiogasPower < 0) {
    const extraPowerNeeded = Math.abs(remainingBiogasPower);
    if (isAddingBiomass) {
      // Calculate additional biomass required
      const biogasRequired = extraPowerNeeded / (22626 * 0.0002973071 * 0.35); // Reversing the power generated formula roughly.
      let multiplier = 1;
      if (cattleType === 'Dairy') multiplier = 0.18 * ((0.396 * 400) + (0.16 * 0.181 * 497));
      else if (cattleType === 'Beef') multiplier = 0.18 * ((0.393 * 400) + (0.16 * 0.121 * 497));
      else multiplier = 0.18 * ((0.356 * 400) + (0.16 * 0.17 * 497));

      const additionalBiomass = biogasRequired / multiplier;
      return {
        status: 'warning',
        title: 'Biomass Insufficient',
        message: `Add ${additionalBiomass.toFixed(2)} tons of biomass to meet the power demand.`,
        surplus: remainingBiogasPower,
        biogasUsed: biogasGeneratedKwh,
        extraPowerNeeded
      };
    } else {
      return {
        status: 'error',
        title: 'Biomass Insufficient',
        message: `Use diesel generator to produce ${extraPowerNeeded.toFixed(2)} kWh of energy.`,
        surplus: remainingBiogasPower,
        biogasUsed: biogasGeneratedKwh,
        extraPowerNeeded
      };
  ...
  return {
    status: 'success',
    title: 'Biogas Sufficient',
    message: 'Connect biogas power system in synchronous mode.',
    surplus_biogas: remainingBiogasPower,
    biogasUsed: biogasPowerNeeded
  };
}
